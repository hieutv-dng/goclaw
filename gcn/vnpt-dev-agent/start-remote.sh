#!/usr/bin/env bash
# start-remote.sh — Run MCP server + expose via supergateway + ngrok
# Usage: ./start-remote.sh [port]
...
echo "{\"mcpServers\":{\"vnpt-dev-agent\":{\"url\":\"$NGROK_URL/mcp\"}}}"
echo ""
echo "--- Claude Code config ---"
echo "{\"mcpServers\":{\"vnpt-dev-agent\":{\"command\":\"npx\",\"args\":[\"-y\",\"supergateway@$SUPERGATEWAY_VERSION\",\"--streamableHttp\",\"$NGROK_URL/mcp\"]}}}"
echo ""
echo "WARNINGS:"
